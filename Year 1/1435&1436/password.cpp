// This program tests a password for correct format
...
int main()
{
	...
	cout << "Your password has "  << countLetters(passWord) << " letters and " << countDigits(passWord) << " digits.";
...
int countLetters(char *strPtr)
{
	...
	{
		//Fill in the code that counts the number of letters
		if (isalpha(*strPtr)){
		    occurs++;
		}
		strPtr++;
...
int countDigits(char *strPtr)
{
	...
	while (*strPtr != '\0')
	{
		//Fill in the code that counts number of digits
		if (isdigit(*strPtr)){
		    occurs++;
		}
		strPtr++;
